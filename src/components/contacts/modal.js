...
class modal extends Component{
  ...
  QueryLawyerName=()=>{
    this.props.form.validateFields((errors,values) => {
      ...
      this.props.onQueryLawyerName(values.contactName)
    })
  }
  ...
  handleConfirmUsers = (rule, value, callback) => {
    fetch(apisource +'/api/crm/contactsName/'+value,{
      method:'get',
      xhrFields:{
    ...
    return (
      <Modal {...modalOpts} footer={[
        ...
        <Form layout="horizontal">
          ...
          {contactInfo?<div style={{textAlign:'center',color:'red',marginTop:'-10px',marginBottom:'10px'}}>{contactInfo.id!=null&&contactInfo.contactType==null&&modalType!='update'?'该手机号已是联系人':''}</div>
            :''}
          {contactInfo?<div style={{textAlign:'center',color:'red',marginTop:'-10px',marginBottom:'10px'}}>{contactInfo.id!=null&&contactInfo.contactType==1?'该手机号已是协作律师，是否进行角色转换？':''}
            {contactInfo.id!=null&&contactInfo.contactType==1?<Button type="primary" onClick={onLawyerOk}>确定</Button>:''}
          </div>
            :''}
          <FormItem label='姓名：' hasFeedback {...formItemLayout}>
            ...
            })(<Input maxLength={64} onBlur={this.QueryLawyerName}/>)}
          </FormItem>
          <FormItem label="对应客户:" hasFeedback {...formItemLayout}>
            {getFieldDecorator('customerId',{
              initialValue: contactInfo &&contactInfo.customerId ? contactInfo.customerId.toString() :'',
              rules: [
                {
                  required: true,
                  message: '对应客户未填写'
