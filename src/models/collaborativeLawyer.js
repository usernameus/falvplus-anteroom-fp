import {queryEdit,queryModal,update,queryChannel,removeChannel,query,createLawyer,updateLawyer,querylawyerHave} from '../services/contacts';
import {message} from 'antd'

export default {
  namespace: 'collaborativeLawyer',
  state: {
    modalVisible: false,
    modalType: 'create',
    saving: false,
  ...
  reducers: {
    ...
    queryLawyerHaveSuccess(state, action){
  ...
  effects: {
    ...
    *create({payload}, {call,put}) {
      try{
        const data = yield call(createLawyer, payload.params)
    ...
    *update({payload}, {call,put}){
      try{
        const data = yield call(updateLawyer,payload.params)
        if (data && data.success) {
          yield put({type: 'hideModal'})
          if(payload.action){
            yield put({type: payload.action, payload: payload.actionPayload})
